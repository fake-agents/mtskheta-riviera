  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-4 items-center">
      ...
      <div className="relative group">
        {/* Tooltip */}
        <div className="absolute top-1/2 -translate-y-1/2 right-16 bg-[#0a1526] border border-[#1877F2]/60 text-white text-xs font-bold px-3.5 py-2 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none whitespace-nowrap shadow-2xl">
          Chat on Messenger
        </div>
        
        {/* Pulsing ring in blue */}
        <div className="absolute inset-0 bg-[#1877F2] rounded-full animate-ping opacity-60 duration-1000"></div>
        
        <a
          href="https://www.facebook.com/MtskhetaRiviera"
          target="_blank"
          rel="noopener noreferrer"
          className="relative flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 bg-[#1877F2] text-white border-2 border-white/30 rounded-full transition-transform duration-200 active:scale-95 hover:scale-105 shadow-[0_8px_30px_rgba(24,119,242,0.5)]"
          aria-label="Facebook Messenger"
        >
          <svg className="w-8 h-8 sm:w-9 sm:h-9 drop-shadow-md" fill="currentColor" viewBox="0 0 24 24">
            <path fillRule="evenodd" d="M12 2C6.477 2 2 6.145 2 11.26c0 2.923 1.542 5.513 3.934 7.194v3.523l3.593-1.972A10.742 10.742 0 0012 20.52c5.523 0 10-4.145 10-9.26S17.523 2 12 2zm1.096 12.35l-2.827-3.024-5.51 3.024 6.068-6.442 2.89 3.024 5.447-3.024-6.068 6.442z" clipRule="evenodd" />
          </svg>
        </a>
      </div>
      ...
      <div className="relative group">
        {/* Tooltip */}
        <div className="absolute top-1/2 -translate-y-1/2 right-16 sm:right-20 bg-[#0e2a1d] border border-[#c9a84c]/60 text-[#f5f0e8] text-xs font-bold px-3.5 py-2 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none whitespace-nowrap shadow-2xl">
